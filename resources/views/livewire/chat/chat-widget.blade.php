...
>
    ...
    <button 
        ...
        class="cursor-pointer flex items-center gap-3 px-2 py-2 rounded-full font-medium transition focus:outline-none
                {{ request('sort') === 'chat' ? 'bg-purple-50 text-purple-600' : 'text-gray-700 hover:bg-gray-100 hover:text-purple-600' }}"
    >
    ...
    >
        ...
        <div class="w-1/3 border-r border-gray-100 flex flex-col bg-gray-50/50">
            <div class="px-4 h-12 flex items-center justify-between border-b border-gray-100 bg-white">
                <h3 class="font-bold text-gray-800">Chats</h3>
                <div class="flex gap-2">
                    {{-- Tombol New Chat --}}
                    <button wire:click="openNewChat" class="p-1 hover:bg-gray-200 rounded text-gray-500 hover:text-black transition cursor-pointer">
                        <x-lucide-plus class="w-4 h-4"/>
                    </button>
                    <a href="{{ route('chat') }}">
                        <button class="p-1 hover:bg-gray-200 rounded text-gray-500 hover:text-black transition cursor-pointer">
            ...
            <div class="flex-1 overflow-y-auto p-2 space-y-1">
                ...
                @foreach($this->conversations as $chat)
                    <button 
                        ...
                        {{ $activeUserId == ($chat->id == auth()->id() ? $chat->sender_id : $chat->id) ? 'bg-purple-50' : '' }}"
                    >
                        ...
                        <div class="flex-1 min-w-0 overflow-hidden">
                            <div class="flex justify-between items-baseline">
                                <span class="text-sm font-medium text-gray-900 truncate">{{ $chat->name }}</span>
        ...
        <div class="flex-1 flex flex-col bg-white relative">
            ...
            @if($view === 'welcome')
                <div class="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
                    ...
                    <div>
                        <h2 class="text-lg font-bold text-gray-900">Welcome to chat!</h2>
                        <p class="text-sm text-gray-500 mt-1 max-w-[200px] mx-auto">Start a direct or group chat with other redditors.</p>
                    </div>
            ...
            @if($view === 'new_chat')
                <div class="h-full flex flex-col">
                    <div class="px-4 h-12 flex items-center justify-between border-b border-gray-100">
                        <h3 class="font-bold text-gray-800">New Chat</h3>
                ...
                <div class="h-full flex flex-col">
                    ...
                    <div class="px-4 h-12 flex items-center justify-between border-b border-gray-100 bg-white">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="font-bold text-sm text-gray-800">{{ $this->activeUser->name ?? 'User' }}</span>
                        </div>
                        <div class="flex gap-2 text-gray-500">
                            <button 
                                wire:click="clearChat"
                                wire:confirm="Bersihkan chat? Pesan akan hilang dari tampilan Anda."
                                class="cursor-pointer hover:bg-red-50 hover:text-red-600 p-1.5 rounded transition"
                                title="Delete for Me"
                            >
                                <x-lucide-trash-2 class="w-4 h-4"/>
                            </button>

                            <button wire:click="closeRoom" class="cursor-pointer hover:bg-gray-100 p-1 rounded transition"><x-lucide-x class="w-4 h-4"/></button>
                        </div>
                    </div>
                    ...
                    <div 
                        ...
                        class="flex-1 bg-white p-4 overflow-y-auto flex flex-col gap-3 scrollbar-hide"
                    >
